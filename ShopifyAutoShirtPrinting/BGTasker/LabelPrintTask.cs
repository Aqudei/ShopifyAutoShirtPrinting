...
{
    public class LabelPrintTask : IBGTask
    {
        ...
        private async Task<string> DownloadRemoteFileToLocalAsync(string source, string destination)
        {

            return await DownloadRemoteFileToLocalAsync(new Uri(source), destination);
        }

        private async Task<string> DownloadRemoteFileToLocalAsync(Uri source, string destination)
        {

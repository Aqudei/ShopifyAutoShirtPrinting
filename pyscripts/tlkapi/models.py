...
class LineItemQueryset(models.QuerySet):
    ...
    def archived_items(self):
        virtual_variants = Variant.objects.filter(product__is_virtual=True).values_list('shopify_id',flat=True)
        return self.filter(Status='Archived').exclude(VariantId__in=virtual_variants)

    def active_items(self):
        virtual_variants = Variant.objects.filter(product__is_virtual=True).values_list('shopify_id',flat=True)
        return self.exclude(Status='Archived').exclude(VariantId__in=virtual_variants)
...
class OrderInfo(models.Model):
    ...
    OriginalBinNumber = models.IntegerField(_("Orig Bin Number"), default=0)
    # Field name made lowercase.
    OrderId = models.BigIntegerField(
...
class PrintRequest(models.Model):
    LineItem = models.ForeignKey(LineItem, verbose_name=_(
        "LineItem"), on_delete=models.CASCADE)
    Done = models.BooleanField(_("Done"), default=False)
    Timestamp = models.DateTimeField(
        _("Timestamp"), auto_now=False, auto_now_add=True)
    Variant = models.ForeignKey(Variant, verbose_name=_("Variant"), on_delete=models.SET_NULL, null=True,blank=True)
